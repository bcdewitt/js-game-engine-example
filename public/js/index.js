...
const _GameEvent$1 = new WeakMap();
let propagatingEvent$1 = null; // Keeps track of the currently propagating event to cancel out duplicate events
...
class GameEvent$1 {
	 ...
	 */
	stopPropagation() {
		_GameEvent$1.get(this).propagate = false;
		return this
	}
	 ...
	 */
	stopImmediatePropagation() {
		const _this = _GameEvent$1.get(this);
		_this.propagate = false;
		_this.propagateImmediate = false;
		return this
	}
 ...
 * @mixin eventTargetMixin
 */
const eventTargetMixin$1 = {
	construct() {
		_eventTargetMixin$1.set(this, {
			listeners: new Map(),
			listenerOptions: new WeakMap(),
			parent: null,
		});
	...
	addEventListener(type, listener, options) {
		const _this = _eventTargetMixin$1.get(this);
		const set = _this.listeners.has(type) ? _this.listeners.get(type) : new Collection$1();
		set.add(listener);
	 ...
	 */
	removeEventListener(type, listener) {
		const set = _eventTargetMixin$1.get(this).listeners.get(type);
		if (!set) return
		set.delete(listener);
		return this
	},
	 ...
	 */
	propagateEventsFrom(child) {
		const _child = _eventTargetMixin$1.get(child);
		if (_child) _child.parent = this;
		return this
	},
...
const _Collection$1 = new WeakMap();

/**
 * Class that extends Set/Collection to create and access subsets via indexes.
 */
class IndexedCollection$1 extends Collection$1 {
	constructor() {
		super();
		_Collection$1.set(this, {
			indexed: new Map(),
			indexers: new Map(),
		});
	}
	 ...
	 */
	setIndex(indexName, indexer) {
		const _this = _Collection$1.get(this);
	 ...
	 */
	getIndexed(indexName) {
		return _Collection$1.get(this).indexed.get(indexName)
	}
	 ...
	 */
	unindexItem(item) {
		const map = _Collection$1.get(this).indexed;
		map.forEach((indexer, key) => {
			map.get(key).delete(item);
		});
		return this
...
class Factory$1 {
	 ...
	 */
	use(middlewareFunc) {
		_Factory$1.get(this).middleware.add(middlewareFunc);
		return this
	}
	...
	set(constructName, construct) {
		const constructNames = Array.isArray(constructName) ? constructName : [ constructName ];
		constructNames.forEach((constructName) => {
			_Factory$1.get(this).constructors.set(constructName, construct);
		});
		return this
	}
	 ...
	 */
	has(constructName) {
		return _Factory$1.get(this).constructors.has(constructName)
	}
	 ...
	 */
	create(constructName, data) {
		const _this = _Factory$1.get(this);
		const construct = _this.constructors.get(constructName);
		if (!construct) {
			console.warn(`${constructName} constructor doesn't exist`);
			return
...
const _AsyncFactory$1 = new WeakMap();

/**
 * A class that serves as a container for asynchronous constructors.
 */
class AsyncFactory$1 {
	constructor() {
		_AsyncFactory$1.set(this, {
			middleware: new Set(),
			constructors: new Map(),
		});
	}
	 ...
	 */
	use(middlewareFunc) {
		_AsyncFactory$1.get(this).middleware.add(middlewareFunc);
		return this
	}
	...
	set(constructName, construct) {
		const constructNames = Array.isArray(constructName) ? constructName : [ constructName ];
		constructNames.forEach((constructName) => {
			_AsyncFactory$1.get(this).constructors.set(constructName, construct);
		});
		return this
	}
	 ...
	 */
	has(constructName) {
		return _AsyncFactory$1.get(this).constructors.has(constructName)
	}
	 ...
	 */
	async create(constructName, data) {
		const _this = _AsyncFactory$1.get(this);
		const construct = _this.constructors.get(constructName);
		if (!construct) {
			console.warn(`${constructName} constructor doesn't exist`);
			return
...
const ADD_PROPS_METHOD_NAME$1 = 'construct';

// Creates a function that "extracts" an object with only the properties that test true
const createExtractObjFunc$1 = test => obj => {
	const extractedObj = {};
	Object.entries(obj).forEach(([key, val]) => {
		if (test(val, key)) extractedObj[key] = val;
	});
...
// Extracts an object with only the non-function properties
const extractConstructor$1 = createExtractObjFunc$1((val, key) => key === ADD_PROPS_METHOD_NAME$1);

// Extracts an object with only the function properties (methods)
const extractMethods$1 = createExtractObjFunc$1((val, key) => key !== ADD_PROPS_METHOD_NAME$1);
 ...
 * @returns {function} - Function that applies mixins to a new class extending Clazz.
 */
const createMixinFunc$1 = (Clazz) => (...mixins) => {
	const constructors = mixins.map(mixin => extractConstructor$1(mixin).construct);
	const methodsMixins = mixins.map(mixin => extractMethods$1(mixin));
 ...
 * @param {...Object} - Mixin objects
 */
const MixedWith$1 = createMixinFunc$1();
...
const createArray$1 = (...args) => {
	...
	let i = length;
	if (args.length > 1) {
		while(i--) arr[length-1 - i] = createArray$1(...(args.slice(1)));
	}

	return arr
};

class ObservableChangeEvent$1 extends GameEvent$1 {
	constructor(type, { prop, args } = {}) {
		super(type);
		Object.defineProperty(this, 'prop', { value: prop, writable: false });
		Object.defineProperty(this, 'args', { value: args, writable: false });
...
const handledObjs$1 = new WeakMap();
 ...
 * @mixes eventTargetMixin
 */
const observableMixin$1$1 = {
	construct() {
		eventTargetMixin$1.construct.call(this);
	},
	 ...
	 */
	makeObservable(prop) {
		if (!handledObjs$1.has(this))
			handledObjs$1.set(this, new Set());
		const handledProps = handledObjs$1.get(this);
		...
		if (typeof val === 'function') {
			Reflect.defineProperty(this, prop, Object.assign({
				get() {
					return (...args) => {
						this.dispatchEvent(new ObservableChangeEvent$1('observableChange', { prop, args }));
						return val.call(this, ...args)
					}
				},
				set(inVal) { val = inVal; }
		...
		// Observe property changes
			...
			Reflect.defineProperty(this, prop, Object.assign({
				get() { return val },
				set(inVal) {
					this.dispatchEvent(new ObservableChangeEvent$1('observableChange', { prop, args: [ inVal ] }));
					val = inVal;
				}
			}));
		}
...
var observableMixin$1$2 = Object.assign({}, eventTargetMixin$1, observableMixin$1$1);

const _Entity$1 = new WeakMap(); // Store private variables here

/**
 * Class representing an Entity (the "E" in the ECS design pattern).
 * @mixes observableMixin
 */
class Entity$1 extends MixedWith$1(observableMixin$1$2) {
	...
	setComponent(compName, component) {
		component.setParentEntity(this);
		_Entity$1.get(this).components.set(compName, component);
		return this
	}
	 ...
	 */
	removeComponent(compName) {
		_Entity$1.get(this).components.delete(compName);
		return this
	}
	 ...
	 */
	getComponent(compName) {
		return _Entity$1.get(this).components.get(compName)
	}
	 ...
	 */
	hasComponent(compName) {
		return _Entity$1.get(this).components.has(compName)
	}
}

const _Component$1 = new WeakMap(); // Store private variables here

/**
 * Class representing a Component (the "C" in the ECS design pattern).
 * @mixes observableMixin
 */
class Component$1 extends MixedWith$1(observableMixin$1$2) {
	 ...
	 */
	setParentEntity(parentEntity = null) {
		_Component$1.get(this).parentEntity = parentEntity;
		return this
	}
	 ...
	 */
	getParentEntity() {
		return _Component$1.get(this).parentEntity
	}
}

class GameStopEvent$1 extends GameEvent$1 {
	constructor(type) {
		super(type, { bubbles: true });
	}
}

class GameSceneChangeEvent$1 extends GameEvent$1 {
	constructor(type, { sceneName } = {}) {
		super(type, { bubbles: true });
		Object.defineProperty(this, 'sceneName', { value: sceneName, writable: false });
	}
...
class SystemUpdateEvent$1 extends GameEvent$1 {
	constructor(type, { entities, deltaTime, timestamp } = {}) {
		super(type);
		Object.defineProperty(this, 'entities', { value: entities, writable: false });
		Object.defineProperty(this, 'timestamp', { value: timestamp, writable: false });
...
class SystemLoadEvent$1 extends GameEvent$1 {
	constructor(type, { assetFetcher } = {}) {
		super(type);
		Object.defineProperty(this, 'assetFetcher', { value: assetFetcher, writable: false });
	}
...
class SystemLoadedEvent$1 extends GameEvent$1 {
	constructor(type, { assets } = {}) {
		super(type);
		Object.defineProperty(this, 'assets', { value: assets, writable: false });
	}
...
// Creates a function that throws an error when run
const unimplemented$1 = name => () => {
	throw new Error(`${name} not set`)
};

const _System$1 = new WeakMap();

/**
 * Class representing a System.
 * @mixes eventTargetMixin
 */
class System$1 extends MixedWith$1(eventTargetMixin$1) {
	constructor() {
		super();
		_System$1.set(this, {
			getEntitiesFunc: unimplemented$1('getEntitiesFunc'),
			addEntityFunc: unimplemented$1('addEntitiesFunc'),
		});
	}
	 ...
	 */
	setGetEntitiesFunc(func) {
		_System$1.get(this).getEntitiesFunc = func;
		return this
	}
	 ...
	 */
	unsetGetEntitiesFunc() {
		_System$1.get(this).getEntitiesFunc = unimplemented$1('getEntitiesFunc');
		return this
	}
	 ...
	 */
	getEntities(indexName) {
		return _System$1.get(this).getEntitiesFunc(indexName)
	}
	 ...
	 */
	setAddEntityFunc(func) {
		_System$1.get(this).addEntityFunc = func;
		return this
	}
	 ...
	 */
	unsetAddEntityFunc() {
		_System$1.get(this).addEntityFunc = unimplemented$1('addEntityFunc');
		return this
	}
	 ...
	 */
	addEntity(entity) {
		_System$1.get(this).addEntityFunc(entity);
		return this
	}
	 ...
	 */
	stopGame() {
		this.dispatchEvent(new GameStopEvent$1('stopGame'));
		return this
	}
	 ...
	 */
	changeScene(sceneName) {
		this.dispatchEvent(new GameSceneChangeEvent$1('changeScene', { sceneName }));
		return this
	}
	 ...
	 */
	load(assetFetcher) {
		return this.dispatchEventAsync(new SystemLoadEvent$1('load', { assetFetcher }))
	}
	 ...
	 */
	loaded(assets) {
		return this.dispatchEventAsync(new SystemLoadedEvent$1('loaded', { assets }))
	}
	 ...
	 */
	update(entities, deltaTime, timestamp) {
		this.dispatchEvent(new SystemUpdateEvent$1('update', { entities, deltaTime, timestamp }));
		return this
	}
}

class SceneLoadEvent$1 extends GameEvent$1 {
	constructor(type, { assetFetcher } = {}) {
		super(type, { bubbles: true });
		Object.defineProperty(this, 'assetFetcher', { value: assetFetcher, writable: false });
	}
...
class SceneLoadedEvent$1 extends GameEvent$1 {
	constructor(type, { assets } = {}) {
		super(type, { bubbles: true });
		Object.defineProperty(this, 'assets', { value: assets, writable: false });
	}
...
class SceneUpdateEvent$1 extends GameEvent$1 {
	constructor(type, { entities, deltaTime } = {}) {
		super(type, { bubbles: true });
		Object.defineProperty(this, 'entities', { value: entities, writable: false });
		Object.defineProperty(this, 'timestamp', { value: deltaTime, writable: false });
...
const _Scene$1 = new WeakMap();

const MAX_UPDATE_RATE$1 = 60 / 1000;

/**
 * Class representing a Scene.
 * @mixes eventTargetMixin
 */
class Scene$1 extends MixedWith$1(eventTargetMixin$1) {
	...
	constructor() {
		...
		_Scene$1.set(this, {
			systems: new Map(),
			entities: new IndexedCollection$1(),
			lastUpdate: null,
		});
	}
	 ...
	 */
	addEntity(entity) {
		if (entity) _Scene$1.get(this).entities.add(entity);
		return this
	}
	 ...
	 */
	addEntities(entities) {
		const innerEntities = _Scene$1.get(this).entities;
		entities.forEach(entity => innerEntities.add(entity));
		return this
	}
	 ...
	 */
	removeEntity(entity) {
		_Scene$1.get(this).entities.delete(entity);
		return this
	}
	 ...
	 */
	hasEntity(entity) {
		return _Scene$1.get(this).entities.has(entity)
	}
	 ...
	 */
	setEntityIndexer(indexName, indexer) {
		_Scene$1.get(this).entities.setIndex(indexName, indexer);
		return this
	}
	 ...
	 */
	reindexEntity(entity) {
		_Scene$1.get(this).entities.reindexItem(entity);
		return this
	}
	...
	getEntities(indexName) {
		...
		return _Scene$1.get(this).entities.getIndexed(indexName)
	}
	 ...
	 */
	getSystem(systemName) {
		return _Scene$1.get(this).systems.get(systemName)
	}
	 ...
	 */
	hasSystem(systemName) {
		return _Scene$1.get(this).systems.has(systemName)
	}
	...
	update(timestamp) {
		...
		if (deltaTime === 0 || deltaTime >= MAX_UPDATE_RATE$1) {
			_this.systems.forEach(system => system.update(entities, deltaTime, timestamp));
			this.dispatchEvent(new SceneUpdateEvent$1('update', { entities, deltaTime, timestamp }));
			_this.lastUpdate = timestamp;
		}
...
const _Game$1 = new WeakMap();

/**
 * Class representing a Game.
 * @mixes eventTargetMixin
 */
class Game$2 extends MixedWith$1(eventTargetMixin$1) {
	 ...
	 */
	setAssetFetcher(assetFetcher) {
		_Game$1.get(this).assetFetcher = assetFetcher;
		return this
	}
	 ...
	 */
	setSceneFactory(sceneFactory) {
		_Game$1.get(this).sceneFactory = sceneFactory;
		return this
	}
	 ...
	 */
	getScene() {
		return _Game$1.get(this).activeScene
	}
...
class FetchProgressEvent$1 extends GameEvent$1 {
	constructor(type, { progress } = {}) {
		super(type, { bubbles: true });
		Object.defineProperty(this, 'progress', { value: progress, writable: false });
	}
...
const IMAGE_EXTENSIONS$1 = Object.freeze(['jpg', 'jpeg', 'gif', 'bmp', 'png', 'tif', 'tiff']);
const AUDIO_EXTENSIONS$1 = Object.freeze(['ogg', 'wav', 'mp3']);
const VIDEO_EXTENSIONS$1 = Object.freeze(['m3u8', 'webm', 'mp4']);

const rejectIfNotOK$1 = response => {
	if(!response.ok) throw new Error('Response not OK')
	return response
};
const fetchOK$1 = (...args) => fetch(...args).then(rejectIfNotOK$1);
const resolveObj$1 = response => response.json();
const resolveBlob$1 = response => response.blob();
const createBlobResolveFunc$1 = (tagName) => (response) => resolveBlob$1(response)
	.then((blob) => new Promise((resolve) => {
		const objUrl = URL.createObjectURL(blob);
		const obj = document.createElement(tagName);
		obj.src = objUrl;
...
const resolveImage$1 = createBlobResolveFunc$1('img');
const resolveAudio$1 = response => response.arrayBuffer();
const resolveVideo$1 = createBlobResolveFunc$1('video');
const resolveText$1 = response => response.text();

const fetchAsset$1 = (path) => {
	const parts = path.split('.');
	const ext = parts.length !== 0 ? parts[parts.length - 1].toLowerCase() : 'json';

	// Select resolver
	let resolve;
	if (ext === 'json') resolve = resolveObj$1;
	else if (IMAGE_EXTENSIONS$1.includes(ext)) resolve = resolveImage$1;
	else if (AUDIO_EXTENSIONS$1.includes(ext)) resolve = resolveAudio$1;
	else if (VIDEO_EXTENSIONS$1.includes(ext)) resolve = resolveVideo$1;
	else resolve = resolveText$1;

	return fetchOK$1(path).then(resolve)
};

// -------------------------------------------------------------------------

const _AssetFetcher$1 = new WeakMap();
...
class AssetFetcher$1 extends MixedWith$1(eventTargetMixin$1) {
	 ...
	 */
	fetch(path) {
		return fetchAsset$1(path)
	}
}

const defaultData$1 = { tilesets: [], properties: {}, tileWidth: 0, tileHeight: 0 };
class TiledMap$1 {
	constructor() {
		// New object, falling back to defaults
		this.data = Object.assign({}, defaultData$1);
		this.resources = new Map();
		this.basePath = '';
		this.tileTypes = [];
		this.layers = new Map();
	...
	decorate(data) {
		this.data = Object.assign({}, defaultData$1, data);
		return this
	}
	...
	initLayers(layers) {
		...
		const tileLayers = layers.filter(layer => layer.data && layer.type === 'tilelayer');
		this.layers = new Map(tileLayers.map((layer) => {
			const data = createArray$1(layer.width, layer.height);
			let idx = 0;
...
const keyboardInputs$1 = Symbol();

const wasPressed$1 = Symbol();
const held$1 = Symbol();

class DigitalInput$1 {
	constructor() {
		this[wasPressed$1] = false;
		this.held = false;
	}

	get held() { return this[held$1] }
	set held(val) {
		this[held$1] = val;

		if (val && this[wasPressed$1]) {
			this[wasPressed$1] = false;
		}
	}
	...
	get pressed() {
		let held = this.held;
		let pressed = !this[wasPressed$1] && held;
		this[wasPressed$1] = held;
...
class InputManager$1 {
	constructor() {
		this[keyboardInputs$1] = {
			[32]: new DigitalInput$1(), // Space Key
			[37]: new DigitalInput$1(), // Left Arrow
			[39]: new DigitalInput$1(), // Right Arrow

			[214]: new DigitalInput$1(), // GamepadLeftThumbstickLeft
			[205]: new DigitalInput$1(), // GamepadDPadLeft
			[213]: new DigitalInput$1(), // GamepadLeftThumbstickRight
			[206]: new DigitalInput$1(), // GamepadDPadRight
			[195]: new DigitalInput$1()  // A Button
		};

		window.addEventListener('keydown', (event) => {
			let key = this[keyboardInputs$1][event.keyCode];
			if(key) { key.held = true; }
		}, false);
	...
	get jumpButton() {
		let key = this[keyboardInputs$1][195];
		if(key.held) { return key }

		return this[keyboardInputs$1][32]
	}

	get leftButton() {
		let key = this[keyboardInputs$1][205];
		if(key.held) { return key }

		key = this[keyboardInputs$1][214];
		if(key.held) { return key }

		return this[keyboardInputs$1][37]
	}

	get rightButton() {
		let key = this[keyboardInputs$1][206];
		if(key.held) { return key }

		key = this[keyboardInputs$1][213];
		if (key.held) { return key }

		return this[keyboardInputs$1][39]
	}
}
...
const game$1 = {
	 ...
	 */
	createIndexedCollection(...args) {
		return new IndexedCollection$1(...args)
	},
	 ...
	 */
	createAsyncFactory(...args) {
		return new AsyncFactory$1(...args)
	},
	 ...
	 */
	createAssetFetcher(...args) {
		return new AssetFetcher$1(...args)
	},
	 ...
	 */
	createInputManager(...args) {
		return new InputManager$1(...args)
	},
	 ...
	 */
	createEntityFactory(...args) {
		return (new Factory$1(...args)).use((constructorName, data = {}) =>
			({
				componentFactory: this.createComponentFactory(),
				entity: new Entity$1(),
				data,
	 ...
	 */
	createSystemFactory(...args) {
		return (new AsyncFactory$1(...args)).use(async (constructorName, data = {}) =>
			({
				system: new System$1(),
				data,
			})
...
class CameraComponent extends game$1.Component {
	constructor(data) {
		super();
		Object.assign(this, {
			x: 0,
...
var createCamera = (componentType, { data } = {}) =>
	new CameraComponent(data);

class StaticPhysicsBody extends game$1.Component {
	constructor(data) {
		super();
		Object.assign(this, {
			x: 0,
...
var createStaticPhysicsBody = (componentType, { data } = {}) =>
...
var createSpawner = (componentType, { data } = {}) =>
	new SpawnerComponent(data);

class SpawnedComponent extends game$1.Component {
...
var createSpawned = (componentType, { data: { spawnerSource } } = {}) =>
...
class SpriteComponent extends game$1.Component {
	constructor(data) {
		super();
		Object.assign(this, {
			x: 0,
...
var createSprite = (componentType, { data = {} }) =>
	new SpriteComponent(data);

const _entity = Symbol('_x');
const _spriteComp = Symbol('_x');

class SpritePhysicsComponent extends game$1.Component {
	constructor(entity) {
		super();
		this[_entity] = entity;
		this.accX = 0;
...
var createSpritePhysics = (componentType, { data: { entity } } = {}) =>
...
class SpriteSoundComponent extends game$1.Component {
	constructor(data) {
		super();
...
var createSpriteSound = (componentType, { data } = {}) =>
...
var createBeing = (componentType, { data: { type } } = {}) =>
	new BeingComponent(type);

const _state = Symbol('_state');
class StateComponent extends game$1.Component {
